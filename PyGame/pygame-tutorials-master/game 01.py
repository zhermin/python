...
class projectile():
    ...
    def draw(self, win):
        pygame.draw.circle(win, self.color, (self.x,self.y), self.radius) # add ", 1)" to unfill the circle
...
class enemy(object):
    ...
    def draw(self, win):
        ...
        if self.visible:
            ...
            self.hitbox = (self.x + 18, self.y, 28, 57)
            # pygame.draw.rect(win, (255,0,0), self.hitbox, 2)
...
man = player(100, 400, 64, 64)

shootLoop = 0
bullets = []

goblin = enemy(200, 405, 64, 64, 300)
...
while run:
    ...
    for event in pygame.event.get(): # check for any events on the computer
        if event.type == pygame.QUIT: # clicking the X button on top right
            run = False
